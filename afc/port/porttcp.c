...
static CHAR	acTCPIPAddr[20];
static USHORT	usTCPMode;
...
void
xMBTCPClientInit( CHAR *acIPAddr )
{
	if( acIPAddr == 0 )
	{
		usTCPMode = SERVER_MODE;
		strcpy(acTCPIPAddr, "");
	}
	else
	{
		usTCPMode = CLIENT_MODE;
		strncpy(acTCPIPAddr, acIPAddr, 15);
	}
}
...
xMBTCPPortInit( USHORT usTCPPort )
{
	...
	if( usTCPMode == SERVER_MODE )
	{
		serveraddr.sin_addr.s_addr = htonl( INADDR_ANY );
	}
	else
	{
		serveraddr.sin_addr.s_addr = inet_addr( acTCPIPAddr );
	}
